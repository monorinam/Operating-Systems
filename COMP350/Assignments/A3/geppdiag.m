function x = geppdiag(A,b)
% geppdiag.m Gauss elimination with no pivoting, for a matrix with only
...
for k = 1:n % the diagonal entry has zeros below it due to the form of A 
  %do a comparison and interchange
  if(A(len-k+1,k) > A(k,k))
      %only need to interchange two columns
      A([k,len-k+1],k) = A([len-k+1,k],k);
      A([k,len-k+1],len-k+1) = A([len-k+1,k],len-k+1);
      b([k,len-k+1]) = b([len-k+1,k]);
  end
